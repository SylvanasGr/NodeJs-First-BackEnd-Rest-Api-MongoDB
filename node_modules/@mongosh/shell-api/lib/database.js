...
let Database = class Database extends decorators_1.ShellApiWithMongoClass {
    ...
    async _runCommand(cmd, options = {}) {
        return this._mongo._serviceProvider.runCommandWithCheck(this._name, cmd, { ...this._mongo._getExplicitlyRequestedReadPref(), ...await this._baseOptions(), ...options });
    }
    async _runAdminCommand(cmd, options = {}) {
        return this._mongo._serviceProvider.runCommandWithCheck(enums_1.ADMIN_DB, cmd, { ...this._mongo._getExplicitlyRequestedReadPref(), ...await this._baseOptions(), ...options });
    }
    ...
    async enableFreeMonitoring() {
        ...
        let error;
        try {
            result = await this._mongo._serviceProvider.runCommand(enums_1.ADMIN_DB, { getFreeMonitoringStatus: 1 }, await this._baseOptions());
        }
        catch (err) {
            error = err;
        }
